function compare_methods_visual_erf_extended(subjects, stim, woi, ori_surface, loc_surface, varargin)
% stim=dots: -2400 - -2300
% stim=instr: 100 - 200
...
for subj_idx=1:length(subjects)
    ...
    for i=1:length(methods)
        ...
        if strcmp(loc_surface,'white-pial')
            fwhm_new_fname=sprintf('FWHM5.00_ori_%s.loc_white.ds-pial.ds.%s.mat',ori_surface, methods{i});
        end
        ...
        fvals(f_idx,i)=invert_visual_erf_subject_extended(subj_info, methods{i}, ori_surface, loc_surface, method_fname,...
...
out_dir=fullfile('../output',loc_surface,'visual_erf');
if params.mpm_surfs
    out_dir=fullfile(out_dir, 'mpm_surfs');
else
    out_dir=fullfile(out_dir, 't1_surfs');
end
out_file=sprintf('ori_%s.loc_%s.%s_results.mat', ori_surface, loc_surface, stim);
