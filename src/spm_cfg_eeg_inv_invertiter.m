...
cov = cfg_files;
cov.tag = 'cov';
cov.name = 'Channel covariance matrix';
cov.filter = 'mat';
cov.num = [1 1];
cov.val={''};
cov.help = {'Select covariance matrix file (mat file with variable C)'};
...
if isfield(job.isstandard, 'custom')
    ...
    if isfield(job.isstandard.custom,'cov') && length(job.isstandard.custom.cov)>0
        dum=load(char(job.isstandard.custom.cov));
        if ~isfield(dum,'C')
            error('Need to have covariance matrix in matrix C');
        end
        megchans=D.indchantype('MEG','good');
        inverse.QE=eye(length(megchans),length(megchans));
        indices={};
        for i=1:length(megchans)
            chan1=D.chanlabels{megchans(i)};
            idx1=find(strcmp(dum.chan_labels,chan1));            
            indices{end+1}=idx1;
        end
        for i=1:length(megchans)
            idx1=indices{i};
            for j=1:length(megchans)
                idx2=indices{j};
                if length(idx1)>0 && length(idx2)>0
                    inverse.QE(i,j)=dum.C(idx1,idx2);
                end
            end
        end
    end
